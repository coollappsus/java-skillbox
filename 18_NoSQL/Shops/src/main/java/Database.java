...
public class Database {
    ...
    public void addProductToShop(int productNumber, String shop) {
        ...
        if (findProduct != null && findShop != null) {
            Bson getProductName = new Document("products", collectionProducts
                    .find(eq("product number", productNumber))
                    .first().get("name"));
    ...
    public void getProductsCount() {
        ...
        AggregateIterable<Document> result = collectionShops.aggregate(
                Arrays.asList(new Document(
                        "$unwind", new Document("path", "$products")),
                        new Document("$group",
    ...
    public void getAveragePrice() {
        ...
        AggregateIterable<Document> result = collectionShops.aggregate(Arrays.asList(
                new Document("$lookup",
